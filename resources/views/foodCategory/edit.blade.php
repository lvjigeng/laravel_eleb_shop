@extends('layout.default')
    ...
    @section('content')
        ...
        <form class="form-group" method="post" action="{{route('foodCategory.update',['foodCategory'=>$foodCategory])}}" enctype="multipart/form-data">
            <div class="form-group">
                <label for="exampleInputName2">分类名称</label>
                <input type="text" name="name" class="form-control" id="exampleInputName2" placeholder="分类名称" value="{{$foodCategory->name}}">
            </div>

            <div class="form-group">
                <label for="exampleInputName2">分类描述</label>
                <textarea name="description"  cols="10" rows="4" class="form-control">{{$foodCategory->description}}</textarea>
            </div>
